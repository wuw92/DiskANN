...
//! redb neighbor list provider (parallel to `super::rocksdb::neighbor_provider`).
...
use super::{Config, KV_TABLE, delete_key, open_db, put_bytes};

pub struct NeighborProvider<I: VectorId> {
    adjacency_list_index: Database,
    config: Config,
    dim: usize,
...
impl<I: VectorId> NeighborProvider<I> {
    ...
    fn new(max_degree: u32, adjacency_list_index: Database, config: Config) -> Self {
        Self {
        ...
        let g = match value {
            ...
            None => {
                return Err(ANNError::log_index_error(
                    "The redb entry for the vector key is not found",
                ));
            }
        };
    ...
    pub fn set_neighbors(&self, vector_id: I, neighbors: &[I]) -> ANNResult<()> {
        ...
        put_bytes(&self.adjacency_list_index, key, &value)
    ...
    pub fn delete_vector(&self, vector_id: I) -> ANNResult<()> {
        ...
        delete_key(&self.adjacency_list_index, key)
            .map_err(|e| ANNError::log_index_error(format!("redb delete failed: {}", e)))?;
        Ok(())
    }
}
