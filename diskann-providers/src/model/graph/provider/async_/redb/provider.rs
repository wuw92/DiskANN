...
/// # Examples
///
/// The following code demonstrates how to instantiate and use the `RedbProvider` in
/// a number of different scenarios.
///
/// ## Full-Precision Only - No Deletes
///
/// This example demonstrates how to create a `RedbProvider` that only supports
/// full-precision vectors.
/// ```ignore
/// use diskann_providers::model::graph::provider::async_::{
///     bf_tree::{
///         RedbProvider, RedbProviderParameters
///     },
...
///
/// let parameters = RedbProviderParameters {
///     max_points: 5,
...
/// let provider = RedbProvider::<f32, _>::new_empty(
///     parameters,
///     NoStore,
///     NoDeletes,
/// );
...
///
/// // Create a table that supports 5 points and 1 start point.
/// let provider = RedbProvider::<f32>::new_empty(
///     parameters,
///     table,
///     NoDeletes,
/// );
...
///
/// // Create a table that supports 5 points and 1 start point.
/// let provider = RedbProvider::<f32, _, _>::new_empty(
///     parameters,
///     table,
///     TableBasedDeletes,
/// );
/// ```
pub struct RedbProvider<T, Q = QuantVectorProvider, D = NoDeletes>
where
    T: VectorRepr,
{
...
#[derive(Debug, Clone)]
pub struct RedbProviderParameters {
...
pub type Index<T, D = NoDeletes> = Arc<DiskANNIndex<RedbProvider<T, NoStore, D>>>;
pub type QuantIndex<T, Q, D = NoDeletes> = Arc<DiskANNIndex<RedbProvider<T, Q, D>>>;

impl<T, Q, D> RedbProvider<T, Q, D>
where
    T: VectorRepr,
{
    ...
    ///
    /// # Arguments
    /// * `params`: An instance of [`RedbProviderParameters`] collecting shared
    ...
    ///
    /// This is the primary constructor for `RedbProvider`. It creates the provider
...
impl<T, Q> RedbProvider<T, Q, TableDeleteProviderAsync>
where
    T: VectorRepr,
{
...
impl<T, D> RedbProvider<T, QuantVectorProvider, D>
where
    T: VectorRepr,
{
...
impl<T, D> RedbProvider<T, NoStore, D>
where
    T: VectorRepr,
{
    /// Return the number of vector reads for full-precision and quant-vectors respectively
...
/// Allow `&RedbProvider` to implement `IntoIter`
///
impl<T, Q, D> IntoIterator for &RedbProvider<T, Q, D>
where
    T: VectorRepr,
{
    type Item = u32;
...
impl<T, Q, D> DataProvider for RedbProvider<T, Q, D>
where
    T: VectorRepr,
    Q: AsyncFriendly,
    D: AsyncFriendly,
{
    ...
    // The `RedbProvider` uses the identity map for IDs.
    //
    type InternalId = u32;
...
impl<T, Q, D> HasId for RedbProvider<T, Q, D>
where
    T: VectorRepr,
    Q: AsyncFriendly,
    D: AsyncFriendly,
...
impl<'a, T, Q, D> DelegateNeighbor<'a> for RedbProvider<T, Q, D>
where
    T: VectorRepr,
    Q: AsyncFriendly,
    D: AsyncFriendly,
...
///
impl<T, Q> Delete for RedbProvider<T, Q, TableDeleteProviderAsync>
where
    Q: AsyncFriendly,
    T: VectorRepr,
{
...
///
impl<T, D> SetElement<&[T]> for RedbProvider<T, QuantVectorProvider, D>
where
    T: VectorRepr,
    D: AsyncFriendly,
{
...
///
impl<T, D> SetElement<&[T]> for RedbProvider<T, NoStore, D>
where
    T: VectorRepr,
    D: AsyncFriendly,
{
...
/// A trait for setting the start points of a RedbProvider.
///
/// This trait is implemented by `RedbProvider` variants that support setting start points.
/// The `Hidden` parameter ensures that users cannot call `set_start_points` directly;
/// they must go through the `RedbProvider::new` constructor which handles this internally.
...
/// Set start points for the RedbProvider with quantization.
///
...
impl<T, D> StartPoint<T> for RedbProvider<T, QuantVectorProvider, D>
where
    T: VectorRepr,
    D: AsyncFriendly,
{
...
/// Set start points for the RedbProvider without quantization.
///
/// This implementation sets the full-precision vectors for each start point
/// and initializes empty neighbor lists.
impl<T, D> StartPoint<T> for RedbProvider<T, NoStore, D>
where
    T: VectorRepr,
    D: AsyncFriendly,
{
...
/// An accessor for retrieving full-precision vectors from the `RedbProvider`.
///
/// This type implements the following traits:
///
/// * [`Accessor`] for the [`RedbProvider`].
...
where
    ...
    D: AsyncFriendly,
{
    pub(crate) fn new(provider: &'a RedbProvider<T, Q, D>) -> Self {
        Self {
            provider,
...
/// An accessor that retrieves the quantized portion of the [`RedbProvider`].
///
/// This type implements the following traits:
///
/// * [`Accessor`] for the `RedbProvider`.
...
where
    ...
    D: AsyncFriendly,
{
    provider: &'a RedbProvider<T, QuantVectorProvider, D>,
    element: Box<[u8]>,
}
...
where
    ...
    D: AsyncFriendly,
{
    pub(crate) fn new(provider: &'a RedbProvider<T, QuantVectorProvider, D>) -> Self {
        Self {
            provider,
...
where
    ...
    D: AsyncFriendly,
{
    fn new(provider: &'a RedbProvider<T, QuantVectorProvider, D>) -> Self {
        Self { provider }
    }
}
...
impl<T, Q, D> SearchStrategy<RedbProvider<T, Q, D>, &[T]> for FullPrecision
where
    T: VectorRepr,
    Q: AsyncFriendly,
    D: AsyncFriendly + DeletionCheck,
...
impl<T, Q, D> DefaultPostProcessor<RedbProvider<T, Q, D>, &[T]> for FullPrecision
where
    T: VectorRepr,
    Q: AsyncFriendly,
    D: AsyncFriendly + DeletionCheck,
...
impl<T, D> SearchStrategy<RedbProvider<T, QuantVectorProvider, D>, &[T]> for Hybrid
where
    T: VectorRepr,
    D: AsyncFriendly + DeletionCheck,
{
...
impl<T, D> DefaultPostProcessor<RedbProvider<T, QuantVectorProvider, D>, &[T]> for Hybrid
where
    T: VectorRepr,
    D: AsyncFriendly + DeletionCheck,
{
...
// Pruning
impl<T, Q, D> PruneStrategy<RedbProvider<T, Q, D>> for FullPrecision
where
    T: VectorRepr,
    Q: AsyncFriendly,
    D: AsyncFriendly,
...
impl<T, D> PruneStrategy<RedbProvider<T, QuantVectorProvider, D>> for Hybrid
where
    T: VectorRepr,
    D: AsyncFriendly,
{
...
impl<T, Q, D> InsertStrategy<RedbProvider<T, Q, D>, &[T]> for FullPrecision
where
    T: VectorRepr,
    Q: AsyncFriendly,
    D: AsyncFriendly + DeletionCheck,
...
impl<T, D> InsertStrategy<RedbProvider<T, QuantVectorProvider, D>, &[T]> for Hybrid
where
    T: VectorRepr,
    D: AsyncFriendly + DeletionCheck,
{
...
impl<T, Q, D, B> MultiInsertStrategy<RedbProvider<T, Q, D>, B> for FullPrecision
where
    T: VectorRepr,
    Q: AsyncFriendly,
    D: AsyncFriendly + DeletionCheck,
...
impl<T, D, B> MultiInsertStrategy<RedbProvider<T, QuantVectorProvider, D>, B> for Hybrid
where
    T: VectorRepr,
    D: AsyncFriendly + DeletionCheck,
    B: for<'a> Batch<Element<'a> = &'a [T]> + Debug,
{
    ...
    fn finish<Itr>(
        &self,
        _provider: &RedbProvider<T, QuantVectorProvider, D>,
        _ctx: &DefaultContext,
        batch: &std::sync::Arc<B>,
        ids: Itr,
    ) -> impl std::future::Future<Output = Result<Self::Seed, Self::FinishError>> + Send
...
/// Inplace Delete
///
impl<T, Q, D> InplaceDeleteStrategy<RedbProvider<T, Q, D>> for FullPrecision
where
    T: VectorRepr,
    Q: AsyncFriendly,
    D: AsyncFriendly + DeletionCheck,
...
impl<T, D> InplaceDeleteStrategy<RedbProvider<T, QuantVectorProvider, D>> for Hybrid
where
    T: VectorRepr,
    D: AsyncFriendly + DeletionCheck,
{
...
/// Stored parameters for reconstructing a redb-backed provider.
///
/// The `bytes` / `max_record_size` / `leaf_page_size` fields are kept for
/// schema parity with `super::bf_tree::BfTreeParams`; redb does not
/// expose 1:1 equivalents, so they are unused at config-build time.
#[derive(Serialize, Deserialize, Clone)]
pub struct RedbParams {
    pub bytes: usize,
...
impl RedbParams {
    /// Build a redb `Config` from the saved parameters and a file path.
    ...
    pub fn to_config(&self, path: &std::path::Path, is_memory: bool) -> Config {
        if is_memory {
            Config::in_memory().expect("failed to create in-memory redb config")
        } else {
            Config::new(path)
        }
    }
...
pub struct SavedParams {
    ...
    pub max_degree: u32,
    pub prefix: String,
    pub params_vector: RedbParams,
    pub params_neighbor: RedbParams,
...
/// Helper struct for generating consistent file paths for RedbProvider persistence.
/// Centralizes all path patterns to avoid hardcoded strings throughout the codebase.
pub struct RedbPaths;
...
mod tests {
    ...
    async fn test_data_provider_and_delete_interface() {
        ...
        let provider = RedbProvider::new_empty(
